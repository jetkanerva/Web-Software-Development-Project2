...
const getQuizzesByTopic = async({ params, state, response }) => {
...
const showIncorrect = async({ render, state, response, params }) => {
    ...
    const questionId = params.qId;
    console.log(questionId);
    const answer = await quizService.correctAnswer(questionId);
    console.log(answer);

    let correct_answer = "";

    try {
        correct_answer = answer[0].option_text;
    } catch {
        console.log("error getting correct answer")
    }
